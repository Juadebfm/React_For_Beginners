...
// Header Component
const Header = (props) => {
  console.log(props); // {welcome:'Welcome to React JS'}
  return (
    <header>
      <div className="header-wrapper">
        <h1>{props.welcome}</h1>
...
const App = () => {
  return (
    <div className="app">
      <Header welcome="Welcome to React JS" />
    </div>
  );
};
...
const App = () => {
  const greetPeople = () => {
    alert("Welcome to React JS Challenge, 2023");
  };
...
// Function to display time in Mon date, year format eg Oct 4, 2023
const showDate = (time) => {
  const months = [
    "January",
    "February",
...
import React from "react";
import ReactDOM from "react-dom";
import JuliusImage from "./images/Julius.jpg";
...
const App = () => {
  ...
  const user = { ...data.author, image: JuliusImage };
